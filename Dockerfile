...
RUN (cp /etc/munin/apache.conf /etc/apache2/sites-enabled/000-default.conf)
RUN (sed -i 's/^Alias.*/Alias \/ \/var\/cache\/munin\/www\//g' /etc/apache2/sites-enabled/000-default.conf)
RUN (sed -i 's/^ *Allow from localhost.*/        Allow from all\n        Require all granted/g' /etc/apache2/sites-enabled/000-default.conf)
